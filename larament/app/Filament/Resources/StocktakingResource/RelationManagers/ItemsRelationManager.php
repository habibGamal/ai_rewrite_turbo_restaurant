...
namespace App\Filament\Resources\StocktakingResource\RelationManagers;
...
class ItemsRelationManager extends RelationManager
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('product.category')
                    ->label('الفئة')
                    ->relationship('product.category', 'name'),

                Tables\Filters\Filter::make('positive_variance')
                    ->label('فائض')
                    ->query(function (Builder $query) {
                        return $query->whereRaw('real_quantity > stock_quantity');
                    }),

                Tables\Filters\Filter::make('negative_variance')
                    ->label('عجز')
                    ->query(function (Builder $query) {
                        return $query->whereRaw('real_quantity < stock_quantity');
                    }),

                Tables\Filters\Filter::make('no_variance')
                    ->label('لا يوجد فرق')
                    ->query(function (Builder $query) {
                        return $query->whereRaw('real_quantity = stock_quantity');
                    }),
