...
export default function Companies({ orders }: { orders: Order[] }) {
  const makeNewOrder = (values: any) => {
    router.post('/make-order', { type: OrderType.Companies })
  }
  ...
  return (
    <Row gutter={[24, 16]}>
      ...
      {orders.map((order) => (
        <CompaniesOrder key={order.id} order={order} />
      ))}
    </Row>
  )
}

const CompaniesOrder = ({ order }: { order: Order }) => {
  return (
    <Col span={6}>
