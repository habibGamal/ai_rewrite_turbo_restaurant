...
export default function Talabat({ orders }: { orders: Order[] }) {
  const makeNewOrder = (values: any) => {
    router.post(`/make-order`, { type: OrderType.Talabat })
  }
  ...
  return (
    <Row gutter={[24, 16]}>
      ...
      {orders.map((order) => (
        <TalabatOrder key={order.id} order={order} />
      ))}
    </Row>
  )
}

const TalabatOrder = ({ order }: { order: Order }) => {
  return (
    <Col span={6}>
