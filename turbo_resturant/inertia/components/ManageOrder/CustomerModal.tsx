...
import { Customer, Order, Region } from '../../types/Models.js'

export default function CustomerModal({
  customerModal,
  order,
}: {
  customerModal: ReturnType<typeof useModal>
  order: Order
}) {
  const [form] = Form.useForm()
  const regions = usePage().props.regions as Region[]

  const saveCustomer = async (values: any) => {
    try {
      const createCusotmerRes = await axios.post<Customer>('/quick-customer', values)
      ...
      router.post(
        ...
        {
          customerId: createCusotmerRes.data.id,
        },
        {
          onSuccess: (page) => {
  ...
  return (
    <Modal {...customerModal} title="بيانات العميل" footer={null} destroyOnClose>
